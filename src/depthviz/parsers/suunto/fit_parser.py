...
########################################################################################
# file_path = "../../../../tests/data/suunto/FreeDiving_2024-10-16T16_33_30.fit"
# stream = Stream.from_file(file_path)
# decoder = Decoder(stream)
# messages, errors = decoder.read(convert_datetimes_to_dates=False)
# # print(messages.keys())

# fit_type = messages.get("file_id_mesgs")[0].get("type")  # activity
# manufacturer = messages.get("file_id_mesgs")[0].get("manufacturer")  # suunto

# records = messages.get("record_mesgs", [])
# # print("timestamp,depth")
# timestamps = []
# depths = []
# for record in records:
#     timestamp = record.get("timestamp")
...
#         if current_dive is None:
#             dive_number += 1
#             current_dive = {"dive_number": dive_number, "data": [], "max_depth": 0}
#             dive_logs.append(current_dive)
#         # if depth > 0.1:
#         if depth > 1.5:
#             descending = True
...
#         if current_dive is not None:
#             current_dive["data"].append({"timestamp": timestamp, "depth": depth})
#             current_dive["max_depth"] = max(current_dive["max_depth"], depth)
...
#         print(f"Dive {i}: Max Depth: {log['max_depth']}m")
#         for row in log["data"]:
#             print(f"{row['timestamp']}\t{row['depth']}")
#             pass
#         i += 1
#     # pprint(log["data"][:5])
